...
def main():
    try:
        ...
        template_path = os.path.join(script_dir, 'Easy_feature_file.xlsx')
        
        if not os.path.exists(template_path):
            template_path = os.path.join(os.getcwd(), 'server', 'Easy_feature_file.xlsx')
            
        if not os.path.exists(template_path):
            raise FileNotFoundError("Easy_feature_file.xlsx template file not found.")

        dest_path = os.path.join(script_dir, 'Easy Feature file Distillation.xlsx')

        # Copy template to the new destination
        ...
        # Recursive flattener for nested dictionary
        ...
        flat_config = flatten_dict(plant_config)

        row_p_idx = 2
        for key, val in flat_config.items():
            ws_param.cell(row=row_p_idx, column=1, value="Live Benchmarking model")
            ws_param.cell(row=row_p_idx, column=2, value="Distillation_LBM")
        ...
        print(json.dumps({
            "status": "success", 
            "message": "Easy Feature file Distillation updated successfully", 
            "file": dest_path,
            ...
            "params_written": len(flat_config)
        }))
