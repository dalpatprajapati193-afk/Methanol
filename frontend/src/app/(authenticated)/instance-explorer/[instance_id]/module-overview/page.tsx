...
export default async function ModuleOverview({
  ...
  const moduleId = parseInt(instance_id, 10);
  ...
  const filteredInstances = instances.filter(ins => ins.capability?.moduleTypeRel?.moduleTypeId === moduleId);
  const filteredCapabilities = capabilities.filter(cap => cap.moduleTypeRel?.moduleTypeId === moduleId);
