...
export default async function InstanceExplorerOverview() {
  let instances: any[] = [];
  ...
  try {
    ...
    console.warn("[InstanceExplorerOverview Warning]: Database fetch failed, using fallbacks:", err);
  }
  ...
  const mappedCapabilityIds = new Set(instances.map((ins) => ins.capabilityId));
  
  const mappedInstances = instances;
  const unmappedCapabilities = capabilities.filter(
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-text-primary tracking-wide">
          Instance Explorer Overview
        </h1>
        <p className="text-sm text-text-secondary mt-1">
          Select a node from the hierarchy on the left to view specific details, or see the global mapping status below.
        </p>
      </div>
        ...
        {mappedInstances.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {mappedInstances.map((ins) => {
              // Construct the URL path for this instance
